...
class TrainVQ:
    def __init__(self, args):
        ...
        self.syn_recon = Recon(args)
        self.perceptual_loss = LPIPS().eval().to(torch.device("cuda"))

        self.discriminator = Discriminator(args).to(device=args.device)
        self.discriminator.apply(weights_init)
        self.opt_syn, self.opt_disc, self.opt_rec = self.conf_optimizer(args)
    ...
    def conf_optimizer(self, args):
        ...
        opt_rec = torch.optim.Adam(
            list(self.syn_recon.decoder.parameters())+
            list(self.syn_recon.post_quant_conv.parameters()),
            lr=lr, eps=1e-08, betas=(args.beta1, args.beta2)
        )
        opt_disc = torch.optim.Adam(self.discriminator.parameters(), lr=lr, eps=1e-08, betas=(args.beta1, args.beta2))

        return opt_syn, opt_disc, opt_rec
    ...
    def train_model(self, args):
        ...
        recon = self.recon
        lst_recon = ['decoder', 'post_quant_conv'] 
        ...
        syn_recon = self.syn_recon.to(torch.device("cuda"))
        for epoch in range(0, args.epochs):
            with tqdm(range(len(train_loader))) as pbar:
                for i, img in zip(pbar, train_loader):
                    ...
                    f_recon = syn_recon(fake_lat)
                    r_recon = recon(real_lat)
                    ...
                    perc_loss = self.perceptual_loss(real_1,f_recon)
                    perceptual_loss = args.perceptual_loss_factor * perc_loss #+ args.rec_loss_factor * rec_loss
                    ...
                    label.fill_(real)
                    g_fake = self.discriminator(fake_lat)
                    ...
                    wandb.log({'D_loss': gan_loss.mean().cpu().detach().numpy().item(),
                               't_loss': t_loss.cpu().detach().abs().numpy().item(), 
                               'perceptual_loss': perceptual_loss.mean().cpu().detach().numpy().item(),'Epoch': epoch})
                    ...
                    if epoch % 30 == 0 and i == total_samples//args.batch_size-50:
                        ...
                        save_image(real_fake_images,os.path.join("/home/moravapa/Documents/images/tas3",f"task3_{epoch}.jpg"), nrow=4)


                    if epoch % 50 == 0 and i == total_samples//args.batch_size-50:
                        with torch.no_grad():
                            torch.save(checkpoints,os.path.join("/scratch/moravapa/chkpt/task3", f"task3_{epoch}.pt"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='task_3', help='For saving the log for each model')
